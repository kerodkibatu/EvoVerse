...
using System;
using System.Linq;
using Raylib_cs; // Needed for GetAllCells().ToList() if not already present
...
namespace EvoVerse
{
    public class WorldGrid
    {
        ...
        private readonly MorphogenField _morphogenField;
        ...
        public WorldGrid(HexLayout initialLayout, int mapRadius)
        {
            ...
            MapRadius = mapRadius;
            _morphogenField = new MorphogenField(mapRadius * 2 + 1);
        }
        ...
        public void Update()
        {
            _morphogenField.Update();
        }

        public void AddMorphogen(MorphogenType type, Hex position, float amount)
        {
            // Convert hex coordinates to morphogen field coordinates
            int x = position.Q + MapRadius;
            int y = position.R + MapRadius;
            _morphogenField.AddMorphogen(type, x, y, amount);
        }

        public float GetMorphogenLevel(MorphogenType type, Hex position)
        {
            int x = position.Q + MapRadius;
            int y = position.R + MapRadius;
            return _morphogenField.GetMorphogenLevel(type, x, y);
        }

        public Dictionary<MorphogenType, float> GetLocalMorphogenLevels(Hex position)
        {
            int x = position.Q + MapRadius;
            int y = position.R + MapRadius;
            return _morphogenField.GetLocalMorphogenLevels(x, y);
        }
    }
}